...
function updateCardImages() {
  ...
  const cards = db
    .prepare("SELECT id, cardId, imagesLarge FROM Card WHERE imagesLarge LIKE '%images.pokemontcg.io%'")
    .all();
  ...
  for (const card of cards) {
    ...
    if (updateNeeded) {
      db.prepare("UPDATE Card SET imagesLarge = ? WHERE id = ?").run(newSmall, newLarge, card.id);
      updated++;
    }
  }
